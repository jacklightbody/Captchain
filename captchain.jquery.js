	...
	//but its a start
  	...
  	$.fn.captchain = function(settings) {
    	return this.each(function(){
    		...
    		var problem='<label class="captchain-exp">'+config.exp+'</label><span class="captchain-problem"'+rand1+' '+sign+' '+rand2+'</span>';
    		...
    		var error='<br/><span class="captchain-error">'+config.linktext+'</span>';
    		$(this).html(link+error+'<div class="captchain-show">'+problem+input+'</div>');
    		$("#"+this.id+' a.captchain-start').bind({
  				click: function() {
   					$(parentform+' div.captchain-show').show();
   					$(parentform+' a.captchain-start').hide();
